...
function writeYourGenres(genreDB) {
    for (let i = 0; i < 3; i++) {
        // let genreRate = prompt(`Ваш любимый жанр под номером ${i + 1}`);
        // genreDB[i] = genreRate;

        // НО лучше записать сокрашенно, так как переменная создалась и сразу использовалась
        genreDB[i] = prompt(`Ваш любимый жанр под номером ${i + 1}`);
    };
    console.log(personalMovieDB);
};
